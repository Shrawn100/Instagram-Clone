...
router.get('/inbox',verifyAndDecodeToken, asyncHandler(async (req, res) => {
  ...
  const currentUserId = req.authData.user._id; // Get the current user's ID from the request parameters
    ...
    const conversationsWithRecentMessages = await Promise.all(conversations.map(async (conversation) => {
      ...
      const mostRecentMessage = await Message.find({
        $or: [
          { sender: mongoose.Types.ObjectId(currentUserId), receiver: otherUserId },
          { sender: otherUserId, receiver: mongoose.Types.ObjectId(currentUserId) },
        ],
      })
        .sort({ timestamp: -1 })
        .limit(1);
...
router.get('/conversation/:id',verifyAndDecodeToken, asyncHandler(async (req, res) => {
